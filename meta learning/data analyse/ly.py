...
Binary problem:
Question is: Is the image normal? If yes, probability is close to 1.
If no, the probablility is close to 0.
This is because we added label 1 to normal images. 
In summary, probability result close to 1 reflects normal image
and close to 0 reflects damaged image
"""
...
#############################################################
# from keras.preprocessing.image import ImageDataGeneratorImageDataGenerator
...
image_directory = 'current1 ori/'
SIZE = 360
dataset = []  # Many ways to handle data, you can use pandas. Here, we are using a list format.
label = []  # Place holders to define add labels. We will add 0 to all damaged images and 1 to normal.

damaged_images = os.listdir(image_directory + 'damaged/')
for i, image_name in enumerate(
        damaged_images):  # Remember enumerate method adds a counter and returns the enumerate object

    if image_name.split('.')[1] == 'jpg':
        image = cv2.imread(image_directory + 'damaged/' + image_name)
        image = Image.fromarray(image, 'RGB')
        image = image.resize((SIZE, SIZE))
        dataset.append(np.array(image))
        label.append(0)

# Iterate through all images in normal folder, resize to 64 x 64
...
model = Sequential()
model.add(Conv2D(32, (11, 11), strides=4, input_shape=INPUT_SHAPE))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(BatchNormalization())

model.add(Conv2D(64, (3, 3), strides=2, kernel_initializer='he_uniform'))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(BatchNormalization())

model.add(Conv2D(128, (3, 3), strides=2, kernel_initializer='he_uniform'))
model.add(Activation('relu'))
# model.add(MaxPooling2D(pool_size=(2, 2)))
model.add(BatchNormalization())
...
model.compile(loss='binary_crossentropy',
              optimizer=keras.optimizers.SGD(lr=0.0001, momentum=0.9),  # also try adam
              metrics=['acc'])
...
# img 23 is damaged - correctly predicts near 0 probability
# Img 22, damaged, correctly lables (low value) but relatively high value.
# img 24 is normal, correctly predicts as normal
# img 26 is damaged but incorrectly gives high value for prediction, normal.
...
# How do we know how it is doing for damaged vs normal?
################################################################
