...
# path = 'small csv/new Figure/d16 normal1-2.csv'
        ...
        for f in fs:
            ...
            if filename.endswith(rule):
                with open(filename, encoding='UTF-8-sig') as csvfile:
                    list = pd.read_csv(csvfile, usecols=[1]
                                       )
                    list = list.iloc[:]
                    plt.figure()
                    plt.plot(list)
                    data_denoising = wavelet_noising(list)  # 调用小波阈值方法去噪
...
# plt.plot(data_denoising)  # 显示去噪结果
# plt.axis('off')
# plt.savefig('small csv/denoise1/d16 normal1-2.jpg')
# plt.show()
